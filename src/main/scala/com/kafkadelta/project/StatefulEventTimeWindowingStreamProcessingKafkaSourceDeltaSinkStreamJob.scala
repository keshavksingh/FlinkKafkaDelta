...
object StatefulEventTimeWindowingStreamProcessingKafkaSourceDeltaSinkStreamJob {
  val logger = LoggerFactory.getLogger(StatefulEventTimeWindowingStreamProcessingKafkaSourceDeltaSinkStreamJob.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val deltaTablePathSink = "abfss://flink@synapseadlsdeveus01.dfs.core.windows.net/Streams/SalesOrderEventTimeAggregate"
    ...
    val stream: DataStream[SalesOrderEvent] = env.fromSource(kafkaSource, WatermarkStrategy.forMonotonousTimestamps(), "KafkaSource")
    val watermarkStrategy = WatermarkStrategy
      .forBoundedOutOfOrderness[SalesOrderEvent](Duration.ofSeconds(10))
      //.withIdleness(Duration.ofMinutes(5))
      .withTimestampAssigner(new SerializableTimestampAssigner[SalesOrderEvent] {
        override def extractTimestamp(element: SalesOrderEvent, recordTimestamp: Long): Long = {
          val eventTime = element.EventProcessingTime.getTime // Convert Timestamp to Long (epoch millis)
          eventTime
        }
      })

    val assignWaterMarkAndTimestamp : DataStream[SalesOrderEvent] = stream.assignTimestampsAndWatermarks(watermarkStrategy).setParallelism(1)

    val processedStream: DataStream[RowData]= assignWaterMarkAndTimestamp
    ...
    env.execute("AzureEventHubKafkaReadADLSDeltaEventWindowedAggWriteExampleJob")
  }
